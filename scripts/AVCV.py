...
ROOT_DIR = "/Users/adamingemansson/Desktop/presentation_250829/"
...
DETECTION_CONFIG = [
    {"name": "DINO", "file": "Dino_tracks.csv", "color": "red"},
    {"name": "CME", "file": "CME_tracks.csv", "color": "blue"}
    # Add Additional Files
]

# Channel/Volume configuration
CHANNEL_CONFIG = [
    {"name": "AP2", "path": "times", "color": "white"}  # Choose from: "red", "green", "blue", "white", "magenta", "cyan", "yellow", "orange"
...
def process_fluorescence_image(images_with_cmaps, vmax=INTENSITY_CEILING, background_percentile=BACKGROUND_PERCENTILE):
    ...
    for ch_order, (ch_idx, img, cmap_name) in enumerate(images_with_cmaps):
        ...
        intensity_min = img.min()
        intensity_max = img.max()
        percentile_value = np.percentile(img, background_percentile)
        img = np.clip(img, percentile_value,intensity_max)
        img_normalized = (img - intensity_min) / (intensity_max - intensity_min+ 1e-8)
        img_corrected = np.clip(img_normalized, 0, vmax)
...
def plot_full(ax, frame, box_size, ID_comp_df, current_id, base_name, sec_name,
    ...
    if not state.has_tracking or state.click_mode:
        ...
        processed_patch = process_fluorescence_image(patch_channels_data,1)
...
def plot_channel_patches(frame, patch_center, box_size_val, z_tol_val, use_maxproj=True):
    ...
    for i, ch_label in enumerate(state.channel_labels):
        ...
        # Use universal fluorescence processing function
        processed_patch = process_fluorescence_image([(i, patch, state.cmaps[i])],1)
        if processed_patch is not None:
...
def update_plot(*args):
    ...
    if active_channels_data:
        # Use universal fluorescence processing function
        processed_image = process_fluorescence_image(active_channels_data, INTENSITY_CEILING)
